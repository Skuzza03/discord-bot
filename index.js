...
const workStatsChannelId = "1457408149899317349";   // WorkStats (Leader)
const workFile = path.join(__dirname, "workStats.json");

/* ============================================ */

// ----------------- Inventory Functions -----------------
function loadInventory() { return fs.existsSync(inventoryFile) ? JSON.parse(fs.readFileSync(inventoryFile, "utf8")) : {}; }
function saveInventory(data) { fs.writeFileSync(inventoryFile, JSON.stringify(data,null,2)); }
function buildStashText(inventory){
    ...
    for(const cat of categories){
        ...
        for(const [item,qty] of Object.entries(items)) text+=`  - ${item} x${qty}\n`;
        text+="\n";
    }
    return "```"+text+"```";
}
...
// ----------------- WorkStats Functions -----------------
function loadWorkStats(){ return fs.existsSync(workFile)?JSON.parse(fs.readFileSync(workFile,"utf8")):{} }
function saveWorkStats(data){ fs.writeFileSync(workFile,JSON.stringify(data,null,2)); }
...
client.on("messageCreate", async (message)=>{
    ...
    if(message.channel.id === workStatsChannelId){
        ...
        if(command === "!res" && args.length){
            const searchName = args.join(" ").toLowerCase();
            const memberKey = Object.keys(stats).find(k=>k.split('#')[0].toLowerCase()===searchName);
            if(!memberKey){
                const sent = await message.channel.send(`❌ Member not found.`);
                setTimeout(()=>sent.delete().catch(()=>{}),10000);
                return message.delete().catch(()=>{});
            ...
            const embed = new EmbedBuilder()
                .setTitle(`✅ Reset stats for ${stats[memberKey].tag}`)
    ...
    if(message.content.toLowerCase() === "!help"){
        await message.channel.send("```DEPOSIT: item qty W/D/M/O\nWITHDRAW: -item qty W/D/M/O```");
        return;
    }
